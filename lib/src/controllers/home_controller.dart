...
Future<bool> status(status) async {
  ...
  bool res;
  final String url =
      '${GlobalConfiguration().getString('api_base_url')}Api_delivery/statusUpdate/$status/${currentUser.value.id}?$_apiToken';
...
Future<bool> updateLatLong(double latitude, double longitude) async {
  ...
  bool res;
  final String url =
      '${GlobalConfiguration().getString('api_base_url')}Api_delivery/latlongUpdate/$latitude/$longitude/${currentUser.value.id}?$_apiToken';
